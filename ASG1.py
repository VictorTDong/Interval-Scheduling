...
if numberOfArgs < 4 or (int(sys.argv[2]) > int(sys.argv[3])):
    ...
    with open(sys.argv[1], 'r') as file:
        ...
        intervals = [[int(interval) for interval in time] for time in csv.reader(file) ]
        intervals = [i for i in intervals if (i[0] >= start and i[1] <= finish)]
    ...
    for currInterval in intervals:
        if currInterval[0] >= finishTime: # Checks to make sure that the start time doesn't overlap with previous finish time
            """
            Appends the current interval to new list if it doesn't overlap, there is no need to remove 
